...
# limitations under the License.
# =========================================================================
# Modified by Sho (https://github.com/sowwnn)
...
class AttnMidBlock(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, temb: torch.Tensor, context: torch.Tensor | None = None, ident: torch.Tensor = None
    ) -> torch.Tensor:
        # del context
        ...
        if ident != None:
            # print(ident.shape)
            ident = ident.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) 
            ident = ident.expand(hidden_states.shape) 
            hidden_states = hidden_states * ident
        # print(hidden_states.shape)
...
class DiffusionModelUNet(nn.Module):
    ...
    def __init__(
        ...
        #     self.img_embeder = ImageEmbedder(emb_dim)
    ...
    def forward(
        ...
        # if self.emb_dim is not None:
        #     ident = self.img_embeder(ident)


        # 5. mid
        h = self.middle_block(hidden_states=h, temb=emb, context=context, ident=ident)
...
class ImageEmbedder(nn.Module):
    def __init__(self, embed_dim=128):
        ...
        # Sử dụng ResNet18 pretrained làm ví dụ
    ...
    def forward(self, x):
        # x shape: [4, 128, 128, 3]
        x = x.permute(0, 3, 1, 2)  # -> [4, 3, 128, 128]
        x = self.features(x)  # -> [4, 512, 1, 1]
